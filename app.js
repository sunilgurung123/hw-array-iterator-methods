...
//     veryOldInventors.push(inventors[i]);
//   }
// }
...
inventors.map(invertor => {
    inventorNames.push({first: invertor.first, last: invertor.last })
})
...
let sorted = inventors.slice().sort((a, b) => a.year - b.year);
    sorted.forEach(inventor => {
  sortedByBirthYear.push(inventor);
});
// Check your work:
...
let foundInventor = inventors.find(inventor => inventor.first === 'Ada');
    if (foundInventor) {
        inventorNamedAda = foundInventor;
    }
// Check your work:
...
devs.some(dev => {
  if (dev.year <= new Date().getFullYear() - 18) {
    isAdultPresent = true;
  }
});
